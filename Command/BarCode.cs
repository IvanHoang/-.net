...
public class Code128
{
    ...
    /// <returns>图形</returns>
    public Bitmap GetCodeImage(string p_Text, Encode p_Code, bool textFlag)
    {
        ...
        GetViewText(_CodeImage, _ViewText, textFlag);
        return _CodeImage;
    }
    /// <summary>
    /// 获取目标对应的数据
    ...
    private void GetViewText(Bitmap p_Bitmap, string p_ViewText, bool flag)
    {
        if (flag)
        {
...
public static class Code128AHelper
{
    /// <summary>
    /// 创建一维码128A图片(png)
    /// </summary>
    ...
    /// <param name="textFlag">是否显示内容(False/True)</param>
    /// <returns></returns>
    public static bool CreateCodeImg(string code, string path, out string msg, bool textFlag = false)
    {
        msg = "";
        //string filepath = System.Web.HttpContext.Current.Server.MapPath(".");
        //string returnpath = "img/lsh/" + code + ".png";
        path += code + ".png";
        try
        {
            if (code != null && code != "")
            {
                if (!System.IO.File.Exists(path))
                {
                    ...
                    try
                    {
                        System.Drawing.Bitmap imgTemp = _Code.GetCodeImage(code, Code128.Encode.Code128A, textFlag);
                        imgTemp.Save(path, System.Drawing.Imaging.ImageFormat.Png);
                    }
                    catch (Exception ex)
                    {
                        msg = "图片生成失败," + ex.Message.ToString();
                        return false;
                    }
                    if (System.IO.File.Exists(path))
                    {
